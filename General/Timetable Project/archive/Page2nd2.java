//Page2nd1.java: Error free, action listeners for edit btn and save btn of inner boxes implemented but still not satisfactory
...
public class Page2nd extends JFrame// implements ActionListener
{
	...
	Page2nd()
	{
		...
		centerPanel.add(Page2nd.scrollPane, new GBC(0,1,1,5).setWeight(100,100).setIpad(900,600));
...
class ParticularTT
{
	...
	{
		//initialising a statr\ement object
		try
		{
		...
		pTTPanel = new JPanel();
		pTTPanel.setLayout(new GridLayout(5,8));
		for(int d=1; d<=5; d++)
		for(int p=1; p<=9; p++)
		{
	...
	public ActionListener addListener(final SavedInnerBox box, final int d, final int p, final String tableName, final String idName, final String[] tableNames)//when edit button is pressed in innerBox
	{
		return(new ActionListener()
		{
