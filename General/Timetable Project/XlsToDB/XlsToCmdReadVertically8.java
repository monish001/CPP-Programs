...
//Only allow traversal of cells for 2 specific cols which corresponds to the chosen section.
					if(cell.getColumnIndex() < sec*2+2)//4)//sec*2+2
						...
						break;
//set variable cellsFromRow2AreInGroupOf2WithEachOther here.//=true means it's a lab.
					...
					switch(cell.getCellType())
					{
//CELL1:tut,lab, lec if 1st section---CELL2:tut,lec if last section
						case Cell.CELL_TYPE_STRING:
							if(!((cell.getRichStringCellValue().getString()).equals("")))
							if(cell.getColumnIndex()%2 == 0)//if left cell for this section
								session[ROOM] = cell.getRichStringCellValue().getString();
							else//if right cell for this section
								session[TEACHER] = cell.getRichStringCellValue().getString();
							if(!((cell.getRichStringCellValue().getString()).equals("")))
								break;
//CELL 2: lab,mutliSectionClassWhereCurrentSection!=LastOne---CELL 1: mutliSectionClassWhereCurrentSection!=1stOne
						default:
							//if(classForSingleSection == false && currCellRange != null)
							//read from row.getRowNum(), startingColIndexForRow1 in case of cell one
							//else from row.getRowNum(), lastColIndexForRow1 in case of cell 2
							//System.out.println(startingColIndexForRow1+" "+lastColIndexForRow1);
							...
							else if(cellsFromRow2AreInGroupOf2WithEachOther == false)//right cell//if lab then skip this.
							{
								for(Row r_: sheet)
								{
									if(r_.getRowNum() != row.getRowNum())
